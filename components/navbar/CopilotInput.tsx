...
export function CopilotInput({ className = '', compact = false }: CopilotInputProps) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`} role="search" aria-label="Copilot command bar">
      {/* Brand pill */}
      {!compact && (
        <div className="flex shrink-0 items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-[11px] font-bold text-white">
      ...
      >
        ...
        <span className="hidden sm:inline">{isLoading ? 'Analyzing…' : 'Execute'}</span>
      </button>
