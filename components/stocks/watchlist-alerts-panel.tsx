...
export function WatchlistAlertsPanel({ stock, score, upside }: Props) {
  ...
  useEffect(() => {
    try {
      setItems(JSON.parse(window.localStorage.getItem(watchlistKey) || '[]'));
    } catch {
      setItems([]);
    }
  }, []);
  ...
  function addCurrentStock() {
    const nextItem: WatchlistItem = {
      ...
      targetPrice,
      notes,
      alerts: alertValue ? [{ type: alertType, value: alertValue, enabled: true }] : [],
    };
  ...
  function addAlert(symbol: string) {
    updateItem(symbol, { alerts: [...(items.find((item) => item.symbol === symbol)?.alerts || []), { type: alertType, value: alertValue, enabled: true }] });
  }
        ...
        {items.length === 0 ? (
          ...
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[1050px] text-left text-sm">
              <thead className="text-slate-400"><tr><th className="py-2">Symbol</th><th>Price at save</th><th>Current loaded price</th><th>Decision</th><th>Confidence</th><th>Entry plan</th><th>Target</th><th>Notes</th><th>Alerts</th><th></th></tr></thead>
              <tbody>
                {items.map((item) => (
                  <tr key={item.symbol} className="border-t border-white/10 text-slate-200">
                    ...
                    <td><input value={item.targetPrice} onChange={(event) => updateItem(item.symbol, { targetPrice: event.target.value })} className="w-28 rounded-lg border border-white/10 bg-slate-950 px-2 py-1 text-white" /></td>
...
function alertLabel(alert: AlertRule) {
  return `${alert.type.replaceAll('_', ' ')} ${alert.value}`;
}
