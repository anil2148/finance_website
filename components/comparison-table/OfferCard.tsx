  ...
  return (
    <article className="offer-card" aria-label={`${product.bank} ${product.name}`}>
      <div>
        ...
        <p className="mt-1 text-xs font-medium text-slate-500">{bestForByCategory[product.category]}</p>
        <p className="mt-2 text-sm text-slate-600">APR/APY: {product.apr_apy} · Annual Fee: {product.annual_fee}</p>
        <p className="mt-1 text-sm text-emerald-700">Bonus: {product.bonus_offer}</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {product.recommended_flag && <span className="inline-flex rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">Best for most readers</span>}
          <span className="inline-flex rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-800">Reviewed March 2026</span>
        </div>
        <div className="mt-2">
