...
type GenerateBlogMetadataOptions = {
  params: { slug: string };
  region?: BlogRegion;
  basePath?: '/blog' | '/us/blog' | '/india/blog';
};
...
export async function generateBlogMetadata({
  ...
  region,
  basePath = '/blog'
}: GenerateBlogMetadataOptions): Promise<Metadata> {
...
export function BlogArticlePage({ params, region = 'US', basePath = '/blog' }: { params: { slug: string }; region?: BlogRegion; basePath?: '/blog' | '/us/blog' | '/india/blog' }) {
  const { post, decodedSlug, canonicalSlug } = resolveBlogPostFromRoute(params.slug);

  if (post && canonicalSlug && decodedSlug !== canonicalSlug) {
    redirect(`${basePath}/${canonicalSlug}`);
  }
  ...
  const pageSchema = webpageSchema({
    pathname: `${basePath}/${post.slug}`,
    name: post.title,
    description: post.metaDescription ?? post.description
  });
  const crumbsSchema = breadcrumbSchema([
    { name: 'Home', item: '/' },
    { name: 'Blog', item: basePath },
    { name: post.title, item: `${basePath}/${post.slug}` }
  ]);
  ...
  const t = (text: string) => transformFinancialTerms(text, region);
  ...
  return (
    <article className="mx-auto max-w-4xl space-y-8 rounded-xl bg-white p-5 sm:p-6 lg:p-8 dark:bg-neutral-900">
      ...
      <header className="space-y-4">
        <h1 className="text-3xl font-bold leading-tight text-neutral-900 dark:text-neutral-100">{t(post.title)}</h1>
        <p className="max-w-3xl leading-relaxed text-neutral-600 dark:text-neutral-400">{t(post.description)}</p>
        ...
        <SocialShareButtons title={t(post.title)} url={absoluteUrl(`${basePath}/${post.slug}`)} />
      </header>

      <ArticleTrustPanel authorId={post.authorId} reviewedById={post.reviewedById} updatedAt={post.updatedAt} />

      <DecisionSupportPanel
        title={t(decisionPanel.title)}
        tone="blue"
        intro={t(decisionPanel.intro)}
        points={decisionPanel.points.map((point) => ({ ...point, text: t(point.text) }))}
      ...
      <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-500/40 dark:bg-emerald-950/30">
        ...
        <div className="mt-3 grid gap-3 text-sm md:grid-cols-2">
          <article><h3 className="font-semibold text-emerald-900 dark:text-emerald-100">The numbers</h3><p className="mt-1 text-emerald-900 dark:text-emerald-100">{t(moneyImpact.hook)}</p></article>
          <article><h3 className="font-semibold text-emerald-900 dark:text-emerald-100">In practice</h3><p className="mt-1 text-emerald-900 dark:text-emerald-100">{t(moneyImpact.scenario)}</p></article>
          <article><h3 className="font-semibold text-emerald-900 dark:text-emerald-100">How to decide</h3><p className="mt-1 text-emerald-900 dark:text-emerald-100">{t(moneyImpact.decision)}</p></article>
          <article><h3 className="font-semibold text-emerald-900 dark:text-emerald-100">Your next step</h3><p className="mt-1 text-emerald-900 dark:text-emerald-100">{t(moneyImpact.action)}</p></article>
        </div>
      </section>
      ...
      {(() => {
        ...
        return (
          <section className="rounded-xl border border-amber-200 bg-amber-50/70 p-5 dark:border-amber-500/40 dark:bg-amber-950/20">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t(insight.title)}</h2>
            <div className="mt-3 grid gap-3 text-sm md:grid-cols-2">
              <article className="rounded-lg border border-amber-200/70 bg-white/90 p-3 dark:border-amber-500/40 dark:bg-slate-900">
                ...
                <p className="mt-1 text-slate-700 dark:text-slate-300">{t(insight.scenario)}</p>
              </article>
              <article className="rounded-lg border border-amber-200/70 bg-white/90 p-3 dark:border-amber-500/40 dark:bg-slate-900">
                <h3 className="font-semibold text-amber-800 dark:text-amber-300">The rule that holds</h3>
                <p className="mt-1 text-slate-700 dark:text-slate-300">{t(insight.rule)}</p>
              </article>
            </div>
          </section>
        );
