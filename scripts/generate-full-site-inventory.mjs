...
for (const page of walk(appDir)) {
  ...
  inventory.push({
    ...
    current_title: route === '/' ? 'Make smarter money decisions with real numbers' : route.slice(1).replace(/[-/]/g, ' '),
    meta_title: route === '/' ? 'FinanceSphere | Make smarter money decisions with real numbers' : `${route.slice(1).replace(/[-/]/g, ' ')} | FinanceSphere`,
